...
userImage.addEventListener("change", function (e) {
  if (hasTint) {
    oldTintColor = document.querySelector("#tint").attributes["fill"].value;
    oldTintColor2 = document.querySelector("#imagesvg").lastChild.attributes["fill"].value;
    oldTintOpacity = document.querySelector("#tint").attributes["fill-opacity"].valueAsNumber;
    eraseTint();
  }
  if (imgAdded) {
...
function createFittedImage(wRatio, hRatio) {
  // Create a black background to make image pop
  ...
  imgBackground.setAttribute("fill-opacity", "0.5");
