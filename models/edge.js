...
const clipEdges = (tPntA, tPntB, height, width, clippedEdges) => {
  ...
  if (tPntA.y < height && tPntB.y > 0 && tPntB.y != tPntA.y) {
    ...
    if (!(tempEdge.pointA.x < 0 && tempEdge.pointB.x < 0) && !(tempEdge.pointA.x > width && tempEdge.pointB.x > width)) {
      //? check
      ...
      if (tempEdge.pointA.x < 0) {
        var aY =
          tempEdge.pointA.y + ((0 - tempEdge.pointA.x) * (tempEdge.pointB.y - tempEdge.pointA.y)) / (tempEdge.pointB.x - tempEdge.pointA.x);
      ...
      if (tempEdge.pointB.x > width) {
        var bY =
          tempEdge.pointB.y -
          ((tempEdge.pointB.x - width) * (tempEdge.pointB.y - tempEdge.pointA.y)) / (tempEdge.pointB.x - tempEdge.pointA.x);
...
const edgeSort = (edge1, edge2) => {
  ...
  if (top1.x != top2.x) {
    ...
    if ((top1.y - bottom1.y) / (top1.x - bottom1.x) < (top2.y - bottom2.y) / (top2.x - bottom2.x)) {
      //less slope
      return -1;
    } else {
      return 1;
...
const quadClip = (src /*[3]*/, edges, height, width) => {
  var errorP = makePoint((src[0].fX - 2 * src[1].fX + src[2].fX) / 4, (src[0].fY - 2 * src[1].fY + src[2].fY) / 4);
...
const cubicClip = (src /*[4]*/, edges) => {
  var errorP1 = makePoint((src[0].fX - 2 * src[1].fX + src[2].fX) / 4, (src[0].fY - 2 * src[1].fY + src[2].fY) / 4);
  var errorP2 = makePoint((src[1].fX - 2 * src[2].fX + src[3].fX) / 4, (src[1].fY - 2 * src[2].fY + src[3].fY) / 4);
  ...
  if (error <= 0.25) {
    ...
    for (var i = 0; i <= numPoints; i++) {
      var t = i / numPoints;
      var choptX =
        Math.pow(1 - t, 3) * src[0].fX +
        3 * Math.pow(1 - t, 2) * t * src[1].fX +
        3 * Math.pow(t, 2) * (1 - t) * src[2].fX +
        Math.pow(t, 3) * src[3].fX;
      var choptY =
        Math.pow(1 - t, 3) * src[0].fY +
        3 * Math.pow(1 - t, 2) * t * src[1].fY +
        3 * Math.pow(t, 2) * (1 - t) * src[2].fY +
        Math.pow(t, 3) * src[3].fY;
